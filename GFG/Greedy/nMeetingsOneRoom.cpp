...
Easy Accuracy: 43.1% Submissions: 28878 Points: 2
...
class Solution
{
    ...
    int maxMeetings(int start[], int end[], int n)
    {
        // Your code here
        if(n<2) return n;
        vector<vector<int> > schedules(n,vector<int>(2,0));
        for(int i=0; i<n; i++) schedules[i]={end[i], start[i]};
        sort(schedules.begin(), schedules.end());
        int counter = 0;
        int endsAt = -1;
        for(int i=0; i<n; i++){
            if(endsAt < schedules[i][1]) {
                counter++;
                endsAt = schedules[i][0];
            }
            // cout<<endsAt<<" ";
        }
        return counter;

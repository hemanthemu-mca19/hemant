...
/**
Minimum Platforms 
Medium Accuracy: 46.78% Submissions: 48229 Points: 4
...
class Solution{
    ...
    int findPlatform(int arr[], int dep[], int n)
    {
    	...
    	sort(arr, arr+n);
    	sort(dep, dep+n);
    	// assuming 1st train has already arrived and no train has departed
    	// i is iterratind on arr and j on dep trains array
    	int i =1, j=0;
    	int result = 1, platform = 1; 
    	while(i<n){
    	    if(arr[i]<=dep[j]) { // trained arraived before dep of earlier one
    	        platform++;
    	        i++;
    	    }
    	    else {
